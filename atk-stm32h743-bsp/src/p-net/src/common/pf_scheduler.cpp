...
void pf_scheduler_init_handle(pf_scheduler_handle_t *handle, char const *name)
{
	handle->name = name;
	pf_scheduler_reset_handle(handle);
}

char const *pf_scheduler_get_name(pf_scheduler_handle_t const *handle)
{
	return handle->name;
}

uint32_t pf_scheduler_get_value(pf_scheduler_handle_t const *handle)
{
	return handle->timer_index;
}

bool pf_scheduler_is_running(pf_scheduler_handle_t const *handle)
{
	...
	pf_scheduler_handle_t *handle)
{
	...
	if (net->scheduler_timeout_first >= PF_MAX_TIMEOUTS)
	{
		...
		((int32_t)(net->scheduler_timeouts[ix_free].when -
